...
const TransactionHistory = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        const res = await authedFetch('/api/transactions?limit=5');
        if (res.ok) {
          const data = await res.json();
          setTransactions(Array.isArray(data) ? data : (data.transactions ?? []));
        }
      ...
      {loading ? (
        ...
        <View className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          {transactions.map((tx, index) => {
            ...
            const formattedDate = new Date(tx.createdAt).toLocaleDateString('en-KE', {
            ...
            return (
              ...
              >
                ...
                <Text className={`font-bold text-sm ${isCredit ? 'text-emerald-600' : 'text-red-500'}`}>
                  {isCredit ? '+' : '-'}KES {formattedAmount}
                </Text>
              </View>
            );
