...
import {
  Text, View, TouchableOpacity, ScrollView, StatusBar,
...
import axios from "axios";
import * as ImagePicker from "expo-image-picker";
import { authedFetch, getAccessToken } from "./(Auth)/auth";
...
const VerifyKyc = () => {
  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        ...
        // authedFetch handles 401 → redirect to login automatically
      } catch {
        // SESSION_EXPIRED already redirected; network errors let user proceed
      }
    };
    loadUser();
  }, []);
  ...
  const validateForm = (): boolean => {
    const { BusinessCertificate, PinCertificate, NationalId,
            PassportCertificate, BankAccountStatement,
            BusinessPremisePhotograph, imageUri } = form;

    if (!BusinessCertificate || !PinCertificate || !NationalId ||
        !PassportCertificate || !BankAccountStatement || !BusinessPremisePhotograph) {
      setError("Please fill in all fields");
      return false;
    }
    if (!imageUri) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Use getAccessToken — axios doesn't go through authedFetch,
      // so we attach the token manually here.
  ...
  return (
    <ScrollView className="flex-1 bg-emerald-900" keyboardShouldPersistTaps="handled">
      <StatusBar barStyle="light-content" />
      <View className="px-6 pt-20 pb-10">
        <Text className="text-white text-sm font-medium mb-2">PayChain</Text>
        {username ? (
      ...
      <View className="bg-white rounded-t-3xl px-6 pt-8 pb-20 min-h-screen">
        <Text className="text-2xl font-bold text-gray-900 mb-2">KYC Verification</Text>
        <Text className="text-gray-500 text-sm mb-6">
          Provide your business documents to get verified. This is a one-time process.
        </Text>
        ...
        <TouchableOpacity
          className={`flex-row items-center border rounded-lg px-4 py-4 mb-6 ${
            form.imageUri ? "bg-emerald-50 border-emerald-300" : "bg-white border-gray-200"
          }`}
          onPress={handlePickImage}
          activeOpacity={0.7}
        >
          <Text className={`text-base ${form.imageUri ? "text-emerald-700 font-medium" : "text-gray-400"}`}>
            {form.imageUri ? "✅ ID Photo Selected" : "Tap to upload National ID photo"}
          </Text>
        </TouchableOpacity>

        {[
          { label: "National ID Number", field: "NationalId" as const,              kbd: "numeric",       placeholder: "e.g. 12345678" },
          { label: "KRA PIN Certificate", field: "PinCertificate" as const,         kbd: "default",       placeholder: "e.g. A012345678B" },
          { label: "Passport Number",     field: "PassportCertificate" as const,    kbd: "default",       placeholder: "e.g. AB1234567" },
          { label: "Business Reg. No.",   field: "BusinessCertificate" as const,    kbd: "default",       placeholder: "e.g. BN/2024/123456" },
          { label: "Bank Account No.",    field: "BankAccountStatement" as const,   kbd: "numeric",       placeholder: "e.g. 1234567890" },
          { label: "Business Address",    field: "BusinessPremisePhotograph" as const, kbd: "default",    placeholder: "e.g. Westlands, Nairobi" },
        ].map(({ label, field, kbd, placeholder }) => (
          <View key={field}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          {loading ? <ActivityIndicator color="white" style={{ marginRight: 8 }} /> : null}
          <Text className="text-white font-bold text-lg">
            {loading ? "Submitting..." : "Submit KYC Documents"}
          </Text>
        </TouchableOpacity>
